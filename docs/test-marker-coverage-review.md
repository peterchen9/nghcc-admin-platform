# P8 Marker Coverage Review

Updated: 2026-05-31

## Scope

This review is documentation-only. It does not add or change pytest markers, test behavior, production application logic, permission behavior, production workflows, `API_PERMISSION_MODE`, database isolation, or parallel execution.
...
| `tests/smoke/test_health.py` | GET `/api/health/`; asserts DB reports ok. | No direct write seen. | No. | No. | read_only | Candidate `read_only` after confirming health check only pings/reads DB. | Low |
...
| `tests/smoke/test_media.py` | Reads `MEDIA_ROOT` and counts files. | No DB write; no DB access seen. File-system read only. | No. | Yes, global restored media count `>= 33827`. | read_only | Candidate `read_only`; consider separate note that it depends on restored media volume. | Low |
...
| `tests/security/test_api_permission_feature_flag.py` | Reads users, exercises API permission mode and write API response preservation. | `force_login()` possible last-login writes; POST/PUT/DELETE write endpoints are intentionally reached but expected 400/404; report-only logs only. | No. | No. | api_scope / unknown / needs review | Candidate `api_scope`; mark write-endpoint response cases cautiously as `mutating` unless proven no durable writes. | High |
| `tests/security/test_api_permission_log_review.py` | Pure parser/CSV writer tests over strings and `StringIO`. | No DB access or write seen. | No. | No. | api_scope / read_only | Candidate `read_only` and `api_scope`. | Low |
...
| `tests/security/test_csrf_behavior.py` | Settings reload tests plus CSRF-mode client GET/POST login checks. | CSRF login POST with valid token may authenticate and update last-login. | No. | No. | csrf / unknown / needs review | Mark CSRF-mode tests `csrf`; only settings-only cases are `read_only`. Valid login POST needs mutation review. | Medium |
| `tests/security/test_menu_permission_strategy.py` | Uses fake users and request factory; tests menu permission helpers and view wrapper. | No DB access or write seen. | No. | No. | read_only | Candidate `read_only`. | Low |
...
| `csrf` | CSRF-mode tests in `test_csrf_behavior.py`, `test_user_ajax_csrf.py`, and `test_write_api_permissions.py`; token/header presence tests in `test_user_ajax_csrf.py` are CSRF-related but not necessarily CSRF-mode gated. | Do not use `ENABLE_CSRF_PROTECTION=True` selection changes in this phase. |
| `api_scope` | `test_api_scope_storage.py`, `test_api_permission_feature_flag.py`, `test_api_permission_log_review.py`, and possibly write/read API permission matrix docs if the marker is intended to include scope planning/recommendation coverage. | Only `test_api_scope_storage.py` is currently marked at module level. |
