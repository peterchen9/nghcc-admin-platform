...
def sync():
    # Remote DB config
    remote_config = {
        'host': os.getenv('REMOTE_MEMBER_DB_HOST', '172.20.60.241'),
        'port': int(os.getenv('REMOTE_MEMBER_DB_PORT', '3306')),
        'user': os.getenv('REMOTE_MEMBER_DB_USER', 'peter'),
        'password': os.getenv('REMOTE_MEMBER_DB_PASSWORD', ''),
        'database': os.getenv('REMOTE_MEMBER_DB_NAME', 'nads25db_raw'),
        'charset': 'utf8mb4',
        'cursorclass': pymysql.cursors.DictCursor
    }
    if not remote_config['password']:
        raise RuntimeError('REMOTE_MEMBER_DB_PASSWORD is required.')
