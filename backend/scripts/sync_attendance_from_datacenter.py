...
def sync():
    ...
    src_config = {
        'host': os.getenv('DATACENTER_DB_HOST', '127.0.0.1'),
        'port': int(os.getenv('DATACENTER_DB_PORT', '3306')),
        'user': os.getenv('DATACENTER_DB_USER', 'peter'),
        'password': os.getenv('DATACENTER_DB_PASSWORD', ''),
        'database': os.getenv('DATACENTER_DB_NAME', 'datacenter'),
        'charset': 'utf8mb4',
    ...
    dst_config = {
        'host': os.getenv('NADS26_DB_HOST', '127.0.0.1'),
        'port': int(os.getenv('NADS26_DB_PORT', '33069')),
        'user': os.getenv('NADS26_DB_USER', 'root'),
        'password': os.getenv('NADS26_DB_PASSWORD', ''),
        'database': os.getenv('NADS26_DB_NAME', 'nads26db'),
        'charset': 'utf8mb4',
        'cursorclass': pymysql.cursors.DictCursor
    }
    if not src_config['password']:
        raise RuntimeError('DATACENTER_DB_PASSWORD is required.')
    if not dst_config['password']:
        raise RuntimeError('NADS26_DB_PASSWORD is required.')
